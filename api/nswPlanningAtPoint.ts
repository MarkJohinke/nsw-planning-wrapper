...
function badRequest(res: VercelResponse, msg: string) { return res.status(400).json({ ok:false, error: msg }); }
function serverError(res: VercelResponse, msg: string, extra?: any) { console.error("SERVER_ERROR", msg, extra??""); return res.status(500).json({ ok:false, error: msg }); }
...
async function queryArcGisPoint(layerUrl: string, lon: number, lat: number) {
  ...
  const params = new URLSearchParams({ f:"json", geometry:geom, geometryType:"esriGeometryPoint", inSR:"4326",
...
function pick(attrs:any, keys:string[], def:string|null=null){ if(!attrs) return def; for(const k of keys){ if(attrs[k]!=null&&attrs[k]!=="") return String(attrs[k]); } return def; }

async function viaArcGis(lon:number, lat:number){
  const [z, f, h] = await Promise.allSettled([
    queryArcGisPoint(ARCGIS.zoning, lon, lat),
    queryArcGisPoint(ARCGIS.fsr,    lon, lat),
    queryArcGisPoint(ARCGIS.hob,    lon, lat)
  ]);
  const zoning = z.status==="fulfilled" ? pick(z.value, ["ZONE","ZONING","ZoneCode","Zone","LABEL"]) : null;
  const fsr    = f.status==="fulfilled" ? pick(f.value, ["FSR","MAX_FSR","Fsr","RATIO","LABEL"]) : null;
  const hob    = h.status==="fulfilled" ? pick(h.value, ["HOB","HEIGHT","MaxHeight","LABEL"]) : null;
...
async function viaPoint(lon:number, lat:number){
  ...
  if (!r.ok) return { error:"upstream error (Point)", status:r.status, body:await r.text() };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (POINT_API_KEY) {
      ...
      diagnostics.point = p;
      if (!(p as any).error) { zoning=(p as any).zoning ?? null; fsr=(p as any).fsr ?? null; hob=(p as any).hob ?? null; }
    }
    if (!POINT_API_KEY || (!zoning && !fsr && !hob)) {
      const a = await viaArcGis(lonNum, latNum);
      diagnostics.arcgis = a;
      zoning = zoning ?? a.zoning; fsr = fsr ?? a.fsr; hob = hob ?? a.hob;
    }
    ...
    const feasibility:FeasibilityRow[] = [{option:"Duplex",notes:"placeholder"},{option:"RFB 3F",notes:"placeholder"}];

    return res.status(200).json({
      ok:true,
      address: address || "Address TBC",
      coords:{ lat:latNum, lon:lonNum },
      zoning, fsr, hob, overlays, sepp_compliance, feasibility,
      recommendations:[
