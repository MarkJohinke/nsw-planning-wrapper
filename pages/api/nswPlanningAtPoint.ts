...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (Number.isNaN(latNum) || Number.isNaN(lonNum)) return res.status(400).json({ ok:false, error:"lat/lon must be numbers" });
    ...
    console.error("HANDLER CRASH", e?.stack || e);
    return res.status(500).json({ ok:false, error:"handler crash" });
  }
}